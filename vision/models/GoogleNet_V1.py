...
writer = SummaryWriter(log_dir='../logs/GoogLeNet_V1')
...
class GoogLeNet(nn.Module):
    ...
    def __init__(self, num_classes: int=1000, aux_logits: bool=True, transform_input: bool=False,
                 init_weights: Optional[bool] = None, blocks: Optional[List[Callable[..., nn.Module]]] = None,
        ...
        if init_weights is None:
            warnings.warn(
                'The default weight initialization of GoogleNet will be changed in future release of '
                'torchvision. If you with to keep the old behavior (which leads to long initialization times'
...
class InceptionAux(nn.Module):
    def __init__(self, in_channels: int, num_classes: int,
                 conv_block: Optional[Callable[..., nn.Module]]=None, dropout: float=0.7) -> None:
        super().__init__()
        if conv_block is None:
            conv_block = BasicConv2d
...
writer.add_graph(googlenet(), torch.rand(1, 3, 224, 224))
writer.close()
